    ...
    methods: {
        ...
        EditMetasFileForm(event) {
            ...
            let changed = {
                ...
                'localizedMetas': this.selectedFile.metas.localizedMetas,
            }

            this.config.locales.forEach(locale => {
              if(
                typeof this.$refs['edit_metas_modal_title_input_'+locale] !== 'undefined' &&
                typeof this.$refs['edit_metas_modal_title_input_'+locale][0] !== 'undefined'

              ) {
                changed['localizedMetas'][locale] = {
                  'title': this.$refs['edit_metas_modal_title_input_'+locale][0].value,
                  'alt': this.$refs['edit_metas_modal_alt_input_'+locale][0].value,
                  'description': this.$refs['edit_metas_modal_desc_input_'+locale][0].value
                };
              }
            });
            ...
            axios.post(action, {
                ...
                let localizedMetas = data.localizedMetas
                this.showNotif(`${this.trans('edit_metas_success')}`)
                selected.metas = newMetas
                selected.localizedMetas = localizedMetas
            }).catch((err) => {
                console.error(err)
                this.ajaxError()
            })
